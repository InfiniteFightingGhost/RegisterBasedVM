...
| **`JUMP`** | `sBx26`| `JUMP label` | Performs relative jump. <br> $\text{Pc} += \text{sBx26} - 1$ |
...
| **`CALL`** | `ABx` | `CALL method() rStart` | Slides register base pointer and transfers control. <br> $\text{StackFramePush}(\text{Pc}, \text{BasePtr})$ <br> $\text{BasePtr} += \text{rStart}$ <br> $\text{Pc} = \text{MethodTable}[\text{methodIndex}] - 1$ |
| **`RETURN`** | `ABx` | `RETURN rStart rEnd` | Restores calling frame and returns values. Copies return range `[rStart, rEnd]` to callee window start, pops StackFrame, and restores parent `BasePtr` and `Pc`. |
| **`FOR`** | *2 Words*| `FOR rIndex max step < label` | **Word 1 (`ABC`):** Stores `rIndex`, `max`, `step`. <br> **Word 2 (`AsBx`):** Stores comparison operator (`comp`) and relative `jumpOffset`. <br> Increments `rIndex` by `step`. If condition `(rIndex comp max)` is true, jumps: $\text{Pc} += \text{jumpOffset} - 2$. Else, skips the second word: $\text{Pc}++$. |
| **`NEWARR`**| `ABC` | `NEWARR rA size` | Allocates a byte array of `size` on the heap, and writes the resulting address to register `rA`. |
...
| **`SETARR`** | `ABC` | `SETARR rPtr index val` | Writes double `val` into heap array `rPtr` at cell offset `index`. <br> $\text{Heap}[\text{rPtr} + \text{index} \times 8] = \text{val}$ |
...
| **`GETARR`** | `ABC` | `GETARR rA rPtr` | Reads double from heap array `rPtr` at cell offset `r0` and writes to register `rA`. <br> $\text{rA} = \text{Heap}[\text{rPtr} + \text{r0} \times 8]$ |
...
| **`PRINTS`**| `ABC` | `PRINTS opB` | Appends the double value of `opB` to the thread's internal string buffer. |
| **`HALT`** | `ABC` | `HALT` | Halts VM execution. Prints the contents of the thread's internal string buffer. |
