...
namespace Raptor
{
    ...
    public unsafe class VirtualMachine : IDisposable
    {
        ...
        private unsafe delegate* managed<ref VMState, void>[] _managedHostMethodsArray =
            new delegate* managed<ref VMState, void>[1024];
        ...
        {
            if (_instHandle.IsAllocated)
                _instHandle.Free();
            if (_constHandle.IsAllocated)
                _constHandle.Free();
            if (_methodsHandle.IsAllocated)
                _methodsHandle.Free();
            if (_heapHandle.IsAllocated)
                _heapHandle.Free();
            if (_outBufferHandle.IsAllocated)
        ...
        public unsafe void RegisterHostMethod(
            ushort methodIndex,
            delegate* managed<ref VMState, void> del
        )
        {
            if (methodIndex >= _managedHostMethodsArray.Length)
            {
                int newSize = Math.Max(methodIndex + 1, _managedHostMethodsArray.Length * 2);
                var newArray = new delegate* managed<ref VMState, void>[newSize];
                Array.Copy(_managedHostMethodsArray, newArray, _managedHostMethodsArray.Length);
                _managedHostMethodsArray = newArray;
            }
            _managedHostMethodsArray[methodIndex] = del;

            // Ensure LoadProgram sets the FFI bit and fallback delegate works
            if (!_registeredHostMethods.ContainsKey(methodIndex))
            {
                HostFFIDelegate fallback = (ref VMState state) => del(ref state);
                _registeredHostMethods[methodIndex] = fallback;
                if (methodIndex >= _registeredHostMethodsArray.Length)
                {
                    int newSize = Math.Max(methodIndex + 1, _registeredHostMethodsArray.Length * 2);
                    var newArray = new HostFFIDelegate[newSize];
                ...
                _registeredHostMethodsArray[methodIndex] = fallback;
            }
        }
        ...
        public void RegisterHostTable(FFIHostTable table)
        {
            ...
            foreach (var pair in table.UnmanagedCallbacks)
            {
                unsafe
                {
                    RegisterHostMethod(pair.Key, (delegate* managed<ref VMState, void>)pair.Value);
                }
            }
        }
        ...
        public void LoadProgram(VMChunk chunk)
        {
            ...
            _instHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _instructions,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
            _constHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _constants,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
            _methodsHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _methods,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
            _heapHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _heap,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
            _outBufferHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _outBuffer,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
            _regHandle = System.Runtime.InteropServices.GCHandle.Alloc(
                _registers,
                System.Runtime.InteropServices.GCHandleType.Pinned
            );
        ...
        ///</summary>
        public unsafe ExecutionResult RunProfile(
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe ref double Reg(double* RegPtr, uint index)
        {
        ...
        private static unsafe void FlushOutput(ref VMState state)
        {
            ...
            {
                Console.Out.Write(
                    new ReadOnlySpan<char>(state.OutBufferPtr, state.OutBufferOffset)
                );
