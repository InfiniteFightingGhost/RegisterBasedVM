...
public sealed class FFIHostTable
{
    ...
    /// </summary>
    [RequiresUnreferencedCode("Assembly scanning uses reflection to discover RaptorModule types.")]
    ...
    {
        // Skip methods marked with [RaptorIgnore]
        if (method.GetCustomAttribute<RaptorIgnoreAttribute>() != null)
            return;

        // Only process methods marked with [RaptorMethod]
        ...
        // Create the delegate (direct bind or typed wrapper)
    ...
    )
    {
        ...
        return (ref VMState state) =>
        {
            unsafe
            {
                ...
                object?[] poolArray = System.Buffers.ArrayPool<object?>.Shared.Rent(totalParams);
                ...
                try
                {
                    ...
                    if (hasStateParam)
                    {
                        ...
                        for (int idx = 0; idx < totalParams; idx++) invokeArgs[idx] = args[idx];
    ...
    )
    {
        ...
        if (allDoubleParams)
        {
            if (returnType == typeof(double))
            {
                ...
                {
                    case 0:
                        var d0 = (Func<double>)Delegate.CreateDelegate(typeof(Func<double>), instance, method);
                        return (ref VMState state) => { unsafe { state.RegPtr[0] = d0(); } };
                    case 1:
                        var d1 = (Func<double, double>)Delegate.CreateDelegate(typeof(Func<double, double>), instance, method);
                        return (ref VMState state) => { unsafe { state.RegPtr[0] = d1(state.RegPtr[0]); } };
                    case 2:
                        var d2 = (Func<double, double, double>)Delegate.CreateDelegate(typeof(Func<double, double, double>), instance, method);
                        return (ref VMState state) => { unsafe { state.RegPtr[0] = d2(state.RegPtr[0], state.RegPtr[1]); } };
                    case 3:
                        var d3 = (Func<double, double, double, double>)Delegate.CreateDelegate(typeof(Func<double, double, double, double>), instance, method);
                        return (ref VMState state) => { unsafe { state.RegPtr[0] = d3(state.RegPtr[0], state.RegPtr[1], state.RegPtr[2]); } };
                    case 4:
                        var d4 = (Func<double, double, double, double, double>)Delegate.CreateDelegate(typeof(Func<double, double, double, double, double>), instance, method);
                        return (ref VMState state) => { unsafe { state.RegPtr[0] = d4(state.RegPtr[0], state.RegPtr[1], state.RegPtr[2], state.RegPtr[3]); } };
                }
            }
            else if (returnType == typeof(void))
            {
                ...
                {
                    case 0:
                        var a0 = (Action)Delegate.CreateDelegate(typeof(Action), instance, method);
                        return (ref VMState state) => a0();
                    case 1:
                        var a1 = (Action<double>)Delegate.CreateDelegate(typeof(Action<double>), instance, method);
                        return (ref VMState state) => { unsafe { a1(state.RegPtr[0]); } };
                    case 2:
                        var a2 = (Action<double, double>)Delegate.CreateDelegate(typeof(Action<double, double>), instance, method);
                        return (ref VMState state) => { unsafe { a2(state.RegPtr[0], state.RegPtr[1]); } };
                    case 3:
                        var a3 = (Action<double, double, double>)Delegate.CreateDelegate(typeof(Action<double, double, double>), instance, method);
                        return (ref VMState state) => { unsafe { a3(state.RegPtr[0], state.RegPtr[1], state.RegPtr[2]); } };
                    case 4:
                        var a4 = (Action<double, double, double, double>)Delegate.CreateDelegate(typeof(Action<double, double, double, double>), instance, method);
                        return (ref VMState state) => { unsafe { a4(state.RegPtr[0], state.RegPtr[1], state.RegPtr[2], state.RegPtr[3]); } };
                }
            }
        }
        ...
        if (parameters.Length == 1)
        {
            ...
            if (returnType == typeof(double))
            {
                ...
                {
                    var d = (Func<int, double>)Delegate.CreateDelegate(typeof(Func<int, double>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = d((int)state.RegPtr[0]); } };
                }
                if (p0Type == typeof(float))
                {
                    var d = (Func<float, double>)Delegate.CreateDelegate(typeof(Func<float, double>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = d((float)state.RegPtr[0]); } };
                }
                if (p0Type == typeof(bool))
                {
                    var d = (Func<bool, double>)Delegate.CreateDelegate(typeof(Func<bool, double>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = d(state.RegPtr[0] != 0.0); } };
                }
            }
            else if (returnType == typeof(int))
            {
                if (p0Type == typeof(int))
                {
                    var d = (Func<int, int>)Delegate.CreateDelegate(typeof(Func<int, int>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = (double)d((int)state.RegPtr[0]); } };
                }
            }
            else if (returnType == typeof(bool))
            {
                ...
                {
                    var d = (Func<double, bool>)Delegate.CreateDelegate(typeof(Func<double, bool>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = d(state.RegPtr[0]) ? 1.0 : 0.0; } };
                }
            }
            else if (returnType == typeof(void))
            {
                if (p0Type == typeof(int))
                {
                    var a = (Action<int>)Delegate.CreateDelegate(typeof(Action<int>), instance, method);
                    return (ref VMState state) => { unsafe { a((int)state.RegPtr[0]); } };
                }
                if (p0Type == typeof(float))
                {
                    var a = (Action<float>)Delegate.CreateDelegate(typeof(Action<float>), instance, method);
                    return (ref VMState state) => { unsafe { a((float)state.RegPtr[0]); } };
                }
                if (p0Type == typeof(bool))
                {
                    var a = (Action<bool>)Delegate.CreateDelegate(typeof(Action<bool>), instance, method);
                    return (ref VMState state) => { unsafe { a(state.RegPtr[0] != 0.0); } };
                }
            }
        }
        ...
        else if (parameters.Length == 2)
        {
            ...
            if (returnType == typeof(int))
            {
                ...
                {
                    var d = (Func<int, int, int>)Delegate.CreateDelegate(typeof(Func<int, int, int>), instance, method);
                    return (ref VMState state) => { unsafe { state.RegPtr[0] = (double)d((int)state.RegPtr[0], (int)state.RegPtr[1]); } };
                }
            }
        }
